...
final class SmoothModelPlayer extends ModelPlayer {
    private final PartPose standing = new PartPose();
    private final PartPose sneaking = new PartPose();
    private boolean smoothSneakEnabled = true;

    SmoothModelPlayer(float modelSize, boolean smallArms) {
        super(modelSize, smallArms);
    }

    void setSmoothSneakEnabled(boolean smoothSneakEnabled) {
        this.smoothSneakEnabled = smoothSneakEnabled;
    }

    @Override
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks,
                                  float netHeadYaw, float headPitch, float scaleFactor, Entity entity) {
        if (!smoothSneakEnabled || !(entity instanceof EntityPlayer)) {
    ...
    private static final class PartPose {
        ...
        private final Transform bodyWear = new Transform();
        private final Transform rightArmWear = new Transform();
        private final Transform leftArmWear = new Transform();
        private final Transform rightLegWear = new Transform();
        private final Transform leftLegWear = new Transform();

        private void capture(SmoothModelPlayer model) {
            ...
            bodyWear.capture(model.bipedBodyWear);
            rightArmWear.capture(model.bipedRightArmwear);
            leftArmWear.capture(model.bipedLeftArmwear);
            rightLegWear.capture(model.bipedRightLegwear);
            leftLegWear.capture(model.bipedLeftLegwear);
        }

        private void applyInterpolated(SmoothModelPlayer model, PartPose target, float progress) {
            ...
            bodyWear.apply(model.bipedBodyWear, target.bodyWear, progress);
            rightArmWear.apply(model.bipedRightArmwear, target.rightArmWear, progress);
            leftArmWear.apply(model.bipedLeftArmwear, target.leftArmWear, progress);
            rightLegWear.apply(model.bipedRightLegwear, target.rightLegWear, progress);
            leftLegWear.apply(model.bipedLeftLegwear, target.leftLegWear, progress);
        }
    }
